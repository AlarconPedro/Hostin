...
[ApiController]
public class EventoQuartosController : ControllerBase
{
    private readonly IEventoQuartosService _service;

    public EventoQuartosController(IEventoQuartosService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<ActionResult> Add(TbEventoQuarto evento)
    {
        if (evento == null)
        {
    ...
    [HttpPut]
    public async Task<ActionResult> Update(TbEventoQuarto evento)
    {
        if (evento == null)
        {
            return BadRequest("Evento cannot be null.");
