...
{
    private readonly IContasReceberService _service;

    public ContasReceberController(IContasReceberService contasReceberService)
    {
        _service = contasReceberService;
    }
    ...
    [HttpPost]
    public async Task<ActionResult> Add(TbContasReceber conta)
    {
        if (conta == null)
        {
        ...
        try
        {
            ...
            return CreatedAtAction(nameof(GetById), new { id = conta.CtrCodigo }, conta);
        }
    ...
    [HttpPut]
    public async Task<ActionResult> Update(TbContasReceber conta)
    {
        if (conta == null)
        {
            return BadRequest("Conta cannot be null.");
