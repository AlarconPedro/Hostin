...
{
    private readonly ICuponsService _service;

    public CuponsController(ICuponsService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<ActionResult> Add(TbCupon cupom)
    {
        if (cupom == null)
        {
    ...
    [HttpPut]
    public async Task<ActionResult> Update(TbCupon cupom)
    {
        if (cupom == null)
        {
            return BadRequest("Cupom cannot be null.");
